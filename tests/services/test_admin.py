...
class TestUpdateUser:
    async def test_update_email(
        ...
        user_repo.update.return_value = make_user(
            id=1, email="new@test.com", full_name="Updated"
        )
    ...
    async def test_update_not_found(
        self, mock_get_session, user_repo, account_repo, password_provider
    ):
        user_repo.get_by_email.return_value = None
    ...
    async def test_email_conflict_with_another_user(
        self, mock_get_session, user_repo, account_repo, password_provider, make_user
    ):
...
class TestDeleteUser:
    ...
    async def test_last_admin_blocked(
        self, mock_get_session, user_repo, account_repo, password_provider, make_user
    ):
